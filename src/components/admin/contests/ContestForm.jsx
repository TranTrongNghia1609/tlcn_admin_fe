...
const ContestForm = ({ mode = 'create' }) => {
  ...
  const handleSaveContest = async () => {
    ...
    try {
      ...
      const problemsData = addedProblems.map((problem) => ({
      ...
      if (isEditMode) {
        ...
        await addProblemToContest(id, problemsData);
        toast.success('Cập nhật kỳ thi thành công');
      } else {
        // Create new contest
        const response = await createContest(payload);
        const contestId = response.data._id;
        await addProblemToContest(contestId, problemsData);
        toast.success('Tạo kỳ thi thành công');
      }
      
      navigate('/contests');
    } catch (err) {
      toast.error(isEditMode ? 'Không thể cập nhật kỳ thi' : 'Không thể tạo kỳ thi', {
        ...
        <Card className="p-6">
          ...
          <ProblemContest 
            ...
            onProblemUpdated={(data) => {
              ...
              console.log('Problems updated:', data);
            }} 
          />
        </Card>
      </div>
